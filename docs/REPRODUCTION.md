...
### A reduced-scale reproduction of NVIDIA's SLM-agent conversion pipeline — and the twelve things that broke
...
The reproduction succeeded in the narrow sense: a **3.2 GB fine-tuned model matched a 5.8 GB
hybrid** on a live-measured promotion gate and now runs as the default. That result occupies about
a page. The other eleven pages are the failures, because they turned out to be the more useful
output.
...
**The component converted** is the router's *task-difficulty classifier*: given a developer task in
prose, emit one of `SIMPLE / MODERATE / COMPLEX / CRITICAL`. The incumbent was a keyword matcher
with a served 9B model as fallback (5.8 GB). The challenger is a QLoRA-tuned Gemma-4-E2B at 4-bit
(3.2 GB), running in-process with no network dependency.
...
|---|---|---|---|
| Training examples | 10,000–100,000 | **677** | ~7% of the low end. Claims that depend on data scale are **not** tested here. |
| Task type | agentic subtasks (tool calls, multi-step work) | **single-turn classification** | The paper's central claim — SLMs can do *agentic* work — is **not** what this tests. |
...
*pipeline* works end-to-end at small scale and produces a deployable model. It does **not** show
that SLMs replace LLM agents, because the converted component is a classifier, not an agent.
...
## 4. The twelve findings the paper does not contain
...
1. **Choose the converted component by leverage, not by convenience.** The classifier feeds a
   ...
   at 0.931. The pipeline was sound; the target was low-leverage.
...
506 tests, CI on ubuntu/macos/windows × py3.9/3.13. Eval datasets and adapters are gitignored;
...
**Converted, gated, deployed.** The tuned 3.2 GB classifier is the default. The pipeline is
re-runnable end to end. The reproduction is honest about being ~7% scale on a narrower task than
the paper's — and the twelve findings above are, in my judgement, worth more than the model.
